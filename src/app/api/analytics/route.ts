...
export async function GET(request: Request) {
  ...
  // Fetch messages with more comprehensive data
  ...
  // Fetch sparks transactions for spending analysis
  const { data: sparksTransactions, error: sparksError } = await serviceSupabase
  ...
  const priceByUserData = Object.entries(priceByUser)
    ...
    .map(([name, value]) => ({
      name: name.substring(0, 8) + "...", // Anonymize user IDs
      value: parseFloat(value.toFixed(6)),
    }));
  ...
  const modelEfficiencyData = Object.entries(modelEfficiency).map(
    ([name, stats]) => ({
      ...
      avgTokensPerMessage: Math.round(stats.totalTokens / stats.messageCount),
